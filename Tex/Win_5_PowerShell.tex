...
\section{Interpreter poleceń PowerShell}
	\begin{enumerate}
		...
		\item \question{PS E:\textbackslash test$ > $ ls\\
			...
			PS E:\textbackslash test$ > $ get-childitem C:\textbackslash test\textbackslash * -include *.xyz -recurse | remove-item\\
		...
		\item \question{Polecenie:\\"get-childitem C:\textbackslash Kolokwium\textbackslash Main\textbackslash * -include *.kol -recurse | remove-item"\\w Windows PowerShell:}%
		...
		\item \question{Co się stanie po wywołaniu komendy:\\get-process pow* | stop-process}%
		...
		\newpage
		\item \question{Prawidłowa postad pętli for w PowerShell to:}%
		{Nie}{for (i = 1, i -le 10, i++) \{ \}}%
		{Nie}{for (i = 1; i -le 10; i++) \{ \}}%
		{Nie}{for (\$i = 1; \$i $ < $= 10; i++) \{ \}}%
		{Tak}{for (\$i = 1; \$i -le 10; \$i++) \{ \}}
		\item \question{Jaki jest rezultat polecenia interpretera PowerShell:\\
			get-childitem C:\textbackslash Work\textbackslash -recurse | get-acl | where \{ \$\_.Owner -match "Maniek"\}}%
		...
		\item \question{Co otrzymamy po wykonaniu następującej komendy w interpreterze PowerShell:\\
			PS C:\textbackslash $ > $ Get-ChildItem | where \{ \$\_.PslsContainer  \} | Select-Object Name}%

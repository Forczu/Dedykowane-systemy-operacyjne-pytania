...
\begin{enumerate}
	...
	\item \question{Co spowoduje dodanie następującego wpisu do pliku /etc/grub.d/40\_custom \\
		menuentry "Windows" \{ \\
		set root='(hd0,1)' \\
		chainloader + 1 \\
		\} }
	...
	\item \question{Parametr ro w opcjach montowania pliku /etc/fstab oznacza, że:}
	...
	\item \question{Jeżeli nie chcemy, aby konfiguracja zdefiniowana w pewnym skrypcie konfiguracyjnym GRUBA znajdującym się w katalogu /etc/grub.d/ była uwzględniona po wykonaniu polecenia update-grub, należy:}
	...
	\item \question{Plik /boot/grub/grub.cfg:}
	...
	\item \question{Na jednej maszynie zainstalowane są systemy Windows 7 oraz Ubuntu Linux:}%
	...
	\item \question{Plik o domyślnej nazwie „/etc/grub.d/40\_custom” służy do:}%
	...
	\item \question{Aby zmienić ustawienia menu GRUB2 (np. zwiększyć czas oczekiwania na wybór systemu operacyjnego) POWINNO SIĘ:}%
	...
	\item \question{Aby dodać system do listy rozruchowej w programie GRUB2 należy:}%
	...
	\item \question{Jeżeli do plików lub skryptów konfiguracyjnych managera bootwania GRUB wprowadzimy jakieś zmiany, musimy zaktualizować plik grub.cfg za pomocą polecenia:}%
	{Nie}{new-grub}%
	{Tak}{update-grub}%
	{Nie}{grub-update}%
	...
	\item \question{W jaki sposób dodajemy informacje o innych systemach operacyjnych do GRUB2?}%
	{Nie}{GRUB2 sam wykryje wszystkie systemy operacyjne bez konfiguracji}%
	{Tak}{Można nadać prawa wykonywania skryptowi:/etc/grub.d/30\_os-prober. Grub2 podczas aktualizacji wyszuka dostępne systemy operacyjne na dyskach twardych}%
	{Nie}{Należy wykonać polecenie grub2-add-new-os z prawami użytkownika}%
	{Tak}{Do pliku /etc/grub.d/40\_custom dodajemy wpis o systemie, następnie uruchamiamy polecenie sudo update-grub2}
	...
	\item \question{Wskaż poprawne nazewnictwa partycji i dysków w GRUB z odpowiednim dla niego wyjaśnieniem:}%
	...
	\item \question{Zaznacz poprawne odpowiedzi dotyczące pliku /etc/fstab}%
	...
	\item \question{Mówiąc o wymianie danych między systemem Linux a system Windows prawdą jest, że:}%
	...
	\item \question{Co znajduje się w katalogu /dev/disk/by-uuid ?}%
	...
	\item \question{Wskaż poprawne zdania na temat oznaczenia dysku /dev/ssd3"}%
	{Nie}{Jest to czwarta partycja czwartego dysku SATA}%
	{Nie}{Jest to czwarta partycja trzeciego dysku SATA}%
	{Tak}{Jest to trzecia partycja czwartego dysku SATA}%
	{Nie}{Oznaczenie nie jest poprawne}

	\item \question{Plikiem konfiguracyjnym managera bootowania GRUB2 jest :}%
